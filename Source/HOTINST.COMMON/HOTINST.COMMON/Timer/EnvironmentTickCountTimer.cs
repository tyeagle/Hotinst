...
*	All rights reserved
*	文 件 名:	EnvironmentTickCountTimer
...
{
    /// <summary>
    /// EnvironmentTickCount定时器
    /// </summary>
    internal class EnvironmentTickCountTimer : ITimer
    {
        #region 字段
        ...
        /// <summary>
        /// 初始化EnvironmentTickCountTimer实例
        /// </summary>
        public EnvironmentTickCountTimer()
        {
        ...
        private void Run()
        {
            int lngElapsedTime = 0;
            int lngEnd = 0;
            int lngBegin = Environment.TickCount;
            while (_bolIsRunning)
            {
                lngEnd = Environment.TickCount;
                lngElapsedTime = lngEnd - lngBegin;    //计算经过的毫秒数
                if (lngElapsedTime >= m_intInterval)
                {
                    lngBegin = Environment.TickCount;

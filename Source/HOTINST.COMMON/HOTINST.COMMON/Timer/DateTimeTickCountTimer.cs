...
{
    /// <summary>
    /// DateTimeTickCount定时器
    /// </summary>
    internal class DateTimeTickCountTimer : ITimer
    {
        #region 字段
        ...
        /// <summary>
        /// 初始化DateTimeTickCountTimer实例
        /// </summary>
        public DateTimeTickCountTimer()
        {
        ...
        private void Run()
        {
            ...
            long lngEnd = 0;
            long lngBegin = DateTime.UtcNow.Ticks;
            while (_bolIsRunning)
            {
                lngEnd = DateTime.UtcNow.Ticks;
                lngElapsedTime = (lngEnd - lngBegin) / 10000;    //计算经过的毫秒数
                if (lngElapsedTime >= m_intInterval)
                {
                    lngBegin = DateTime.UtcNow.Ticks;

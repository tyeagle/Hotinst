...
namespace HOTINST.OSGI.Core.Root
{
    ...
    /// <summary>
    /// 框架内核，实现了插件的加载、运行、管理，内核也作为一个Bundle，
    /// 框架内核为每个插件创建一个BundleContext对象并初始化插件，其余插件通过BundleContext与Framework通信
    /// </summary>
    internal class Framework : IFramework, IFrameworkFireEvent, IFrameworkService, IFrameworkListener, IFrameworkInstaller
    {
        ...
        /// 框架Bundle自身的上下文对象
        /// </summary>
        private IBundleContext selfBundleContext;

        /// <summary>
        /// Bundle目录，其中每个Bundle有自己的子目录
        /// </summary>
        ...
        /// 共享程序集目录
        /// </summary>
        private string sharedLibsDirectory;

        /// <summary>
        /// Bundle卸载存储目录
        /// </summary>
        private string bundlesUninstalledDirectory;
        ...
        /// <summary>
        /// 已注册的契约服务<服务接口 IList <服务引用>>
        /// </summary>
        private IDictionary<string, IList<IServiceReference>> serviceReferenceDictionary =
            new Dictionary<string, IList<IServiceReference>>();

        /// <summary>
        /// 引用服务的模块, <服务引用  IList<引用服务的模块>>
        /// </summary>
        private IDictionary<IServiceReference, IList<IBundle>> usingServiceBundleDict =
            new Dictionary<IServiceReference, IList<IBundle>>();
        ...
        public Framework()
        {
            this.selfBundleContext = new BundleContext(this, this);
        ...
        private void LoadMetaData()
        {
            ...
            metaDataDictionary.Add(BundleConst.BUNDLE_MANIFEST_DESCRIPTION, description);
        ...
        private void ReadConfig()
        {
            ...
            bundlesDirectory = BundleConfigProvider.GetBundlesDirectory();

            sharedLibsDirectory = BundleConfigProvider.GetShareLibsDirectory();

            bundlesUninstalledDirectory = BundleConfigProvider.GetBundlesUninstallDirectory();
        }
        ...
        private void LoadShareAssemblys()
        {
            ...
            if (Directory.Exists(sharedLibsDirectory))
            {
        ...
        private void DiscoverBundles()
        {
            ...
            BundleConfigProvider.LoadBundlesConfig(bundlesDirectory);
            ...
            foreach (var bundleConfig in BundleConfigProvider.BundlesConfiguration)
            {
                ...
                string bundleDirectoryName = Path.Combine(bundlesDirectory, bundleKey);
                try
                {
        ...
        public void Start()
        {
            ...
            //锁定文件
            if (BundleConfigProvider.HOTINST_OSGI_SINGLERUNNING)
            {
                try
                {
                    string lockFileName = Path.Combine(bundlesDirectory, FRAMEWORK_LOCK_FILE);
                    lockFileStream = File.Open(lockFileName, FileMode.OpenOrCreate, FileAccess.ReadWrite, FileShare.None);
                }
                catch (Exception ex)
                {
                    throw new IOException("锁定Bundle缓存目录失败，请确认没有另外的HOTINST.OSGI实例正在运行或者当前用户有访问此目录的权限。", ex);
                }
            ...
            string cacheBaseDirectory = AppDomain.CurrentDomain.BaseDirectory;
            try
            {
                foreach (IBundle bundle in bundleList)
                {
                    if (bundle.Equals(this) || 255 <= bundle.GetBundleStartLevel()) continue;
                        ...
                        || bundle.GetState() == BundleStateConst.RESOLVED)
                    {
                        ...
                        try
                        {
                            Directory.SetCurrentDirectory(bundle.GetBundleDirectoryPath());
                            bundle.Start();
                        }
                        catch (Exception ex)
                        {
            ...
            {
                Directory.SetCurrentDirectory(cacheBaseDirectory);
            }            
        ...
        public IBundle Install(string zipFile)
        {
            ...
            var newBundlePath = Path.Combine(bundlesDirectory, folderName ?? string.Empty);
            ...
            if (Directory.Exists(newBundlePath))
            {
                ...
                newBundlePath = Path.Combine(bundlesDirectory, folderName);
            }
        ...
        {
            log.Debug(string.Format("卸载模块备份至[{0}]目录！", bundlesUninstalledDirectory));
            ...
            if (!Directory.Exists(bundlesUninstalledDirectory)) Directory.CreateDirectory(bundlesUninstalledDirectory);
            Directory.Move(bundle.GetBundleDirectoryPath(), Path.Combine(bundlesUninstalledDirectory, newbundlePath));
        }
